...
export const searchGmail = tool({
  ...
  execute: async ({ query, maxResults }) => {
    const accessToken = await getAccessToken("google-oauth2");
    ...
    if (!response.ok) {
      ...
      return { error: `Gmail API error: ${response.status} - ${errorText}`, messages: [] };
    }
        ...
        return {
          ...
          subject:
            headers.find((h: { name: string }) => h.name === "Subject")
              ?.value || "(no subject)",
          from:
            headers.find((h: { name: string }) => h.name === "From")
              ?.value || "unknown",
          date:
            headers.find((h: { name: string }) => h.name === "Date")
              ?.value || "",
